...
export async function getProducts(){const snapshot=await getDocs(collection(db,"products"));return snapshot.docs.map(item=>({id:item.id,...item.data()})).filter(product=>product.active!==false)}
